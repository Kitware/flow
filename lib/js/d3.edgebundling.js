...
d3.chart.edgebundling = function(options) {
  ...
  var packageDepends = function (nodes, positive) {
    ...
    nodes.forEach(function(d) {
      if (d.dependsPos) d.dependsPos.forEach(function(i) {
        var source = map[d.name]
        source["is_pos"] = true;
        depends.push({source: source, target: map[i]});
      });
      if (d.dependsNeg) d.dependsNeg.forEach(function(i) {
        var source = map[d.name]
        source["is_pos"] = false;
        depends.push({source: source, target: map[i]});	
      });
  ...
  function filterData(data, setpoint) {
    var filteredData = [];
  ...
  function mouseovered(d) {
    ...
    link
      .classed("link--target", function(l) { if (l.target === d) return l.source.target = true && l.source.is_pos; })
      .classed("link--source", function(l) { if (l.target === d) return l.source.source = true && !l.source.is_pos; })
  ...
  function chart(selection) {
    selection.each(function(data) {
      var setpoint = 0.95;
      var filteredData = filterData(data, setpoint);
