...
d3.chart.heatmap = function(options) {
  function createHeatmap(selection, rowLabelsData) {
    selection.each(function(data) {
      ...
      var select = selection.append("div")
        .append("select")
        .attr("class", "sortingOptionsMenu")
        .on("change", function() { return changeOrder(this.value); });

      var sortingOptions = {none: "---", columnsAndRows: "Initial order on columns and rows", rows: "Initial order on rows", columns: "Initial order on columns"};

      select.selectAll("option")
        .data(d3.keys(sortingOptions))
        .enter().append("option")
        .attr("value", function(d) { return d; })
        .text(function(d) { return sortingOptions[d]; });
