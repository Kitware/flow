...
d3.chart.heatmap = function(options) {
  function createHeatmap(selection, columnHeaders) {
    selection.each(function(data) {
      ...
      var margin = {top: 10, right: 10, bottom: 10, left: 10};  // NOTE: Heatmap position will be adjusted to fit the labels
      ...
      d3.select("#heatmap").style("height", (height + heatmapY + 100) + "px");
      ...
      var cells = row.selectAll(".cells")
        ...
        .attr("fill", function(d) {
          return isNaN(d) ? "white" : colorScale(d);
        })
      ...
      function getMaxWidth(labels) {
        var maxWidth = 0;
        svg.selectAll("text.foo").data(labels)
          .enter().append("text")
          .attr("class", "rowLabel mono")
          .text(function(d) { return d; })
          .each(function(d) {
